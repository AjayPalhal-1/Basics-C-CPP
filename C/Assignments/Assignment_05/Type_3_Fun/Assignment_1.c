...
#include<stdio.h>
     ...
     void   TempF_2_C(double*);              // Q1
     void  Peri_Area_Rectangle (int* ,int* );   // Q2 A)
     void AreaOfCircle(double*,double* );           // Q2 B)
     void reverse(int*);                // Q3 
     void EvnOdd (int);                // Q4 
...
{    double  C=45;
     ...
     int * l= &Len ;
     int * w = &wid;
     Peri_Area_Rectangle (l,w);   //Q2 A)
     double rediuse = 30.0;
     double* R = &rediuse;
     double pi = 3.14;
     double * pay=&pi;
     AreaOfCircle(R,pay);  //Q2 B)
      int num12=789;
      int* num= &num12;
     reverse(num*);                // Q3 
    int num1= 1234 ;
  ...
  // Q1 
void  TempF_2_C (double * C)
{
   
   double F= (9.0/5.0) * (*C) + 32;
   printf("Conversion of Temp %lf Celsius to Fahrenheit is %lf\n\n\n",C,F);
}

// Q2 A) Finding area and perimeter of rectangle.

void Peri_Area_Rectangle (int* l ,int* w)
{
    int area;
    area = (*l)*(*w);
...
void AreaOfCircle(double* R,double*pay )
{
//Finding area and Circumefrence  of  circle.

double Area = (*pay) * ((*R) * (*R) );
double Curcom = 2 * ((*pay) * (*R));
printf("Area of Circle is %lf\n\n",Area); 
printf("Curcom of Circle is %lf\n\n",Curcom); 
}
